...
const MainScreen = () => {
      ...
      function calcPI(numDec) {            //get the nth deciaml value
        ...
        for (i = 0; i <= numDec; i++) {            //checking the value grater then 0
          if (i == 0) sPI += tempPI.charAt(i) ;
            ...
            if (true) addcount = " (" + (i) + ")";   //add rounded count values
            else addcount = "";
  ...
  return (
    ...
    {loader?<ActivityIndicator style={{marginTop:'40%'}} animating={true} color={Colors.red700} size='small'/>:
<View>
    ...
    />
       <Button icon="location-enter" mode="contained" onPress={()=>
        {
          let valid=validator.isNumeric(text)
          if(valid){
          setloader(true)
          setTimeout(() => {
...
<ScrollView style={{paddingHorizontal:'5%',marginBottom:'30%'}}>

  <Text style={{fontSize:20,marginBottom:'5%',color:Colors.indigo900}}>Decimal value of : {text}</Text>



<Text style={{fontSize:20,marginBottom:'5%'}}>{result}</Text>
